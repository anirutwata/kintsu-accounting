...
export async function POST(req: Request) {
  const supabase = await createClient()
  const { bank_name, account_number, account_name, flowaccount_bank_account_id, flowaccount_chart_of_account_id } = await req.json()
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      account_name,
      flowaccount_bank_account_id: flowaccount_bank_account_id || null,
      flowaccount_chart_of_account_id: flowaccount_chart_of_account_id || null,
    })
    .select()
    .single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
